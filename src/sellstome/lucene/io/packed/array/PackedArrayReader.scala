 ...
 *
 * <h3>A sparse array format compression description:</h3>
 ...
 * In case if the gap for ords between adjusted positions is equal to 1 we omit the gap value and simply write the next value
 ...
 * value for a given block in this sense the blocks depends on each other. Q: is it the right design for composability.
 * note: not intended for multi-threaded use
 * Q: should we support the non required dv? It affects the merge (I mean different merge from lucene merge) operation.
